...
class RemoteCmd():
    ...
    def run_job(self, sepc):
        ...
        try:
            ...
            jd.executable      = spec['command']
            jd.arguments       = spec['args']
            jd.output          = spec["stdout"]
            jd.error           = spec["stderr"]
            jd.working_directory = spec.get('pwd', self.WORKING_DIR)
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~Example~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
exece = RemoteCmd(mysql_url='mysql://ensadmin:ensembl@mysql-ens-hive-prod-1:4575/vinay_test1')

spec = {'command': 'init_pipeline.pl ', 
        'args': ['Bio::EnsEMBL::Hive::Examples::LongMult::PipeConfig::LongMult_conf', '-pipeline_url', 'mysql://ensadmin:ensembl@mysql-ens-hive-prod-1:4575/vinay_test1 ', '-hive_force_init', '1'],
         'stdout': 'output.stdout',
         'stderr': 'output.stderr'}

print(exece.run_job(spec)) 
    
spec = {'command': 'beekeeper.pl ',
         'args': [ '-url', 'mysql://ensadmin:ensembl@mysql-ens-hive-prod-1:4575/vinay_test1', '-loop' ],
         'stdout': 'output.stdout',
         'stderr': 'output.stderr'}

print(exece.run_job(spec))
print(exece.beekeper_status())
